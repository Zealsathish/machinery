  ...
  describe "--scope=unmanaged-files" do
    ...
    it "can deal with spaces and quotes in file names" do
      @subject_system.inject_file(
        File.join(Machinery::ROOT, "spec/data/unmanaged_files/SLES-test-quote-char-1.0-1.noarch.rpm"),
        "/tmp/SLES-test-quote-char-1.0-1.noarch.rpm",
        :owner => "root",
        :mode => "644"
      )

      @subject_system.inject_file(
        File.join(Machinery::ROOT, "spec/data/unmanaged_files/RHEL-test-quote-char-1.0-1.noarch.rpm"),
        "/tmp/RHEL-test-quote-char-1.0-1.noarch.rpm",
        :owner => "root",
        :mode => "644"
      )

      @subject_system.run_command(
        "rpm -i /tmp/SLES-test-quote-char-1.0-1.noarch.rpm || rpm -i /tmp/RHEL-test-quote-char-1.0-1.noarch.rpm ",
        as: "root"
      )
